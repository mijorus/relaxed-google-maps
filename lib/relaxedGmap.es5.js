...
var RelaxedMap = /*#__PURE__*/function () {
  ...
  _createClass(RelaxedMap, [{
    ...
    key: "load",
    value: function load() {
      var src = checkData(this.config.src, this.relaxedGMap, 'src');
      ...
      if (!src) {
        ...
        if (inViewport(this.relaxedGMap)) {
          ...
          document.addEventListener('scroll', this.asyncMapLoad(), {
            passive: true
          });
        }
      }
    ...
    key: "asyncMapLoad",
    value: function asyncMapLoad() {
      if (inViewport(this.relaxedGMap)) {
        document.removeEventListener('scroll', {}, {
          passive: true
        });
        var link = this.relaxedGMap.querySelector('.relaxed-map-alt');
        if (link) link.remove();
        this.iframe.style.backgroundImage = '';
        this.iframe.style.filter = '';
        this.iframe.setAttribute('src', this.config.src);
        var mapLoad = new Event('relaxedMapLoad');
        this.relaxedGMap.dispatchEvent(mapLoad);
      }
    }
  }]);
